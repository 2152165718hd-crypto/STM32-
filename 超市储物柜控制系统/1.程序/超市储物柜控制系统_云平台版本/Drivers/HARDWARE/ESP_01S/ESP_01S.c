...
static void ESP_SetWifiState(uint8_t connected);
static uint8_t ESP_ResponseContains(const char *needle);
#if ESP_CLOUD_CLIENT_MODE
static void ESP_HandleCloudConnectionAbort(ESP_ConnState_t failed_state);
#endif
...
static uint8_t ESP_ResponseContains(const char *needle)
{
    ...
    resp_len = at_resp_len;
    if (resp_len < needle_len)
    {
        return 0U;
    }

    for (i = 0U; i <= (uint16_t)(resp_len - needle_len); i++)
    {
        uint16_t j;

        for (j = 0U; j < needle_len; j++)
        {
            if (at_resp_buf[i + j] != (uint8_t)needle[j])
            {
                break;
            }
        }
...
static void ESP_SetWifiState(uint8_t connected)
{
    wifi_connected = (connected != 0U) ? 1U : 0U;
}

#if ESP_CLOUD_CLIENT_MODE
/* Keep the hotspot session alive when only the cloud socket drops. */
static void ESP_HandleCloudConnectionAbort(ESP_ConnState_t failed_state)
{
    if ((failed_state == ESP_CONN_CIPMUX) ||
        (failed_state == ESP_CONN_CIPDINFO) ||
        (failed_state == ESP_CONN_CIPSTART))
    {
        return;
    }

    if ((wifi_connected != 0U) && (failed_state != ESP_CONN_CWJAP))
    {
        return;
    }
...
ESP_Status_t ESP_SendAT(const char *cmd, const char *ack, uint32_t timeout)
{
    ...
    {
        if (ESP_ResponseContains(ack) != 0U)
        {
            at_waiting = false;
            return ESP_OK;
        }

        if ((ESP_ResponseContains("ERROR") != 0U) ||
            (ESP_ResponseContains("FAIL") != 0U) ||
            (ESP_ResponseContains("busy") != 0U))
        {
            at_waiting = false;
...
ESP_Status_t ESP_SendData(uint8_t link_id, const uint8_t *data, uint16_t len)
{
    ...
    {
        if (ESP_ResponseContains("SEND OK") != 0U)
        {
            at_waiting = false;
            return ESP_OK;
        }

        if ((ESP_ResponseContains("SEND FAIL") != 0U) ||
            (ESP_ResponseContains("ERROR") != 0U) ||
            (ESP_ResponseContains("CLOSED") != 0U))
        {
            at_waiting = false;
            ESP_ClearClientConnected();
            return ESP_ERR_SEND_FAIL;
...
static void ESP_ConnectionAbort(void)
{
    ESP_ConnState_t failed_state = conn_state;
    ...
    server_ready = 0U;
#if ESP_CLOUD_CLIENT_MODE
    ESP_HandleCloudConnectionAbort(failed_state);
#else
    ESP_SetWifiState(0U);
#endif
    ESP_ClearClientConnected();
...
#if ESP_CLOUD_CLIENT_MODE
        if (wifi_connected != 0U)
        {
            conn_state = ESP_CONN_CIPMUX;
        }
        else
        {
            conn_state = ESP_CONN_CWJAP;
        }
#else
...
static ESP_AsyncResult_t ESP_PollAsyncAT(void)
{
    ...
    if (ESP_ResponseContains(async_ack) != 0U)
    {
        at_waiting = false;
        async_ack = NULL;
        async_start_tick = 0U;
...
static void ESP_ProcessConnection(void)
{
    ...
    if ((server_ready != 0U) && (tcp_connected != 0U))
    {
        conn_state = ESP_CONN_IDLE;
        return;
    }
...
#if ESP_CLOUD_CLIENT_MODE
    case ESP_CONN_CWJAP:
        snprintf(async_cmd_buf, sizeof(async_cmd_buf), "AT+CWJAP=\"%s\",\"%s\"",
                 ESP_WIFI_SSID, ESP_WIFI_PASS);
        (void)ESP_StartAsyncAT(async_cmd_buf, "WIFI GOT IP", ESP_WIFI_JOIN_TIMEOUT);
        break;
#else
    case ESP_CONN_CWSAP:
...
#if ESP_CLOUD_CLIENT_MODE
    case ESP_CONN_CIPSTART:
        snprintf(async_cmd_buf, sizeof(async_cmd_buf), "AT+CIPSTART=0,\"TCP\",\"%s\",%u",
                 ONENET_MQTT_HOST, (unsigned int)ONENET_MQTT_PORT);
