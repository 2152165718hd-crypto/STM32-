...
static void ST7735_InitAFPin(GPIO_TypeDef *port, uint16_t pin)
{
	...
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
	GPIO_InitStruct.Alternate = ST7735_SPI_GPIO_AF;
	HAL_GPIO_Init(port, &GPIO_InitStruct);
}
#endif
...
static void ST7735_SPIInit(void)
{
	...
	st7735_hdma_tx.Instance = ST7735_SPI_TX_DMA_STREAM;
	st7735_hdma_tx.Init.Channel = ST7735_SPI_TX_DMA_CHANNEL;
	...
	st7735_hdma_tx.Init.FIFOMode = DMA_FIFOMODE_DISABLE;
	st7735_hdma_tx.Init.FIFOThreshold = DMA_FIFO_THRESHOLD_FULL;
	st7735_hdma_tx.Init.MemBurst = DMA_MBURST_SINGLE;
	st7735_hdma_tx.Init.PeriphBurst = DMA_PBURST_SINGLE;
...
static void ST7735_DrawCharRun(uint16_t x, uint16_t y, const char *str, uint16_t char_count, uint16_t color, uint16_t bgColor, const FontDef *font)
{
	uint32_t row;
	uint16_t glyph;
	uint32_t col;
	...
	if ((str == NULL) || (char_count == 0U) || (font == NULL))
	{
		return;
	}

	run_width = (uint16_t)(char_count * font->width);
	mask = (font->width > 16U) ? 0x80000000UL : 0x8000UL;
	ST7735_BeginAddressWindowWrite(x, y, x + run_width - 1U, y + font->height - 1U);

	for (row = 0U; row < font->height; row++)
	{
		for (glyph = 0U; glyph < char_count; glyph++)
		{
			if (!ST7735_GetCharIndex(str[glyph], font, &char_index))
			{
				if (font == &Font_Custom)
				{
					b = 0U;
				...
				{
					char_index = (uint16_t)('?' - 32);
					b = font->data[char_index * font->height + row];
				}
			}
			else
			{
			...
			for (col = 0U; col < font->width; col++)
			{
				...
				pixel_color = ((b << col) & mask) ? color : bgColor;
				st7735_tx_buffer[buffer_used] = (uint8_t)(pixel_color >> 8);
				st7735_tx_buffer[buffer_used + 1U] = (uint8_t)(pixel_color & 0xFFU);
				buffer_used += 2U;
			}
...
{
	size_t char_bytes;
	uint16_t max_chars_per_flush;

	if ((str == NULL) || (font == NULL) || (font->data == NULL) || (font->width == 0U) || (font->height == 0U))
	{
		return;
	}

	char_bytes = (size_t)font->width * font->height * 2U;
	max_chars_per_flush = (uint16_t)(ST7735_TX_BUFFER_SIZE / char_bytes);
	if (max_chars_per_flush == 0U)
	{
		max_chars_per_flush = 1U;
	}

	while (*str != '\0')
	{
		uint16_t chars_on_row;
		...
		chars_on_row = (uint16_t)((st7735_width - x) / font->width);
		if (chars_on_row > max_chars_per_flush)
		{
			chars_on_row = max_chars_per_flush;
		}

		while ((run_count < chars_on_row) &&
			   (str[run_count] != '\0') &&
			   (str[run_count] != '\n') &&
			   (str[run_count] != '\r'))
		{
		...
		ST7735_DrawCharRun(x, y, str, run_count, color, bgColor, font);
		x = (uint16_t)(x + (run_count * font->width));
		str += run_count;
	}
}

void ST7735_DrawGlyph16(uint16_t x, uint16_t y, const uint8_t glyph[32], uint16_t color, uint16_t bgColor)
{
	uint8_t row;
	uint8_t col;
	size_t buffer_used = 0U;

	if ((glyph == NULL) || (x + 16U > st7735_width) || (y + 16U > st7735_height))
	{
		return;
	}

	ST7735_BeginAddressWindowWrite(x, y, x + 15U, y + 15U);
	for (row = 0U; row < 16U; row++)
	{
		uint16_t bits = (uint16_t)(((uint16_t)glyph[row * 2U] << 8U) | glyph[row * 2U + 1U]);
		for (col = 0U; col < 16U; col++)
		{
			uint16_t pixel_color = ((bits & (uint16_t)(0x8000U >> col)) != 0U) ? color : bgColor;
			st7735_tx_buffer[buffer_used] = (uint8_t)(pixel_color >> 8U);
			st7735_tx_buffer[buffer_used + 1U] = (uint8_t)(pixel_color & 0xFFU);
			buffer_used += 2U;
		}
	}
