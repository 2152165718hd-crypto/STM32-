...
static UART_HandleTypeDef huart_esp; /* 模块内部 UART 句柄 */
...
static void ESP_UART_Init(void)
{
    ...
    HAL_GPIO_Init(ESP_PORT, &GPIO_InitStruct);
    ...
    /* UART 参数 */
    huart_esp.Instance = ESP_UART;
    huart_esp.Init.BaudRate = ESP_UART_BAUD;
...
static void ESP_UART_Recover(void)
{
    __HAL_UART_DISABLE_IT(&huart_esp, UART_IT_RXNE);
    __HAL_UART_DISABLE_IT(&huart_esp, UART_IT_PE);
    __HAL_UART_DISABLE_IT(&huart_esp, UART_IT_ERR);

    __HAL_UART_CLEAR_PEFLAG(&huart_esp);
    __HAL_UART_CLEAR_FEFLAG(&huart_esp);
    __HAL_UART_CLEAR_NEFLAG(&huart_esp);
    ...
    at_resp_len = 0;
    at_waiting = false;
    huart_esp.ErrorCode = HAL_UART_ERROR_NONE;
    huart_esp.RxState = HAL_UART_STATE_READY;
    (void)HAL_UART_Receive_IT(&huart_esp, &rx_byte, 1);
}
...
ESP_Status_t ESP_SendAT(const char *cmd, const char *ack, uint32_t timeout)
{
    ...
    /* 发送指令 */
    HAL_UART_Transmit(&huart_esp, (uint8_t *)cmd, strlen(cmd), 1000);
...
ESP_Status_t ESP_SendData(uint8_t link_id, const uint8_t *data, uint16_t len)
{
    ...
    HAL_UART_Transmit(&huart_esp, (uint8_t *)data, len, 2000);
