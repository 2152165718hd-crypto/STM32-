...
static void FM225_RecvCommandSafe(void);
...
static bool FM225_RingBuf_Peek(uint16_t offset, uint8_t *byte)
{
    uint16_t available = FM225_RingBuf_Available();
    if (offset >= available)
    {
        return false;
    }
...
static void FM225_RecvCommandSafe(void)
{
    ...
    {
        uint8_t h0, h1;
        (void)FM225_RingBuf_Peek(0, &h0);
        (void)FM225_RingBuf_Peek(1, &h1);
        if (h0 == (uint8_t)(FM225_START_CODE >> 8) &&
            h1 == (uint8_t)(FM225_START_CODE & 0xFF))
        {
            break;
        }

        (void)FM225_RingBuf_Pop(&byte);
        available--;
    }
    ...
    if (!FM225_RingBuf_Peek(2, &byte))
    {
        return;
    }
    resp_type = (FM225_ResponseType_t)byte;

    if (!FM225_RingBuf_Peek(3, &byte))
    {
        return;
    }
    length = (uint16_t)byte << 8;
    if (!FM225_RingBuf_Peek(4, &byte))
    {
        return;
    }
    length |= byte;

    frame_size = 5U + (uint32_t)length + 1U;
    if (frame_size > FM225_RX_BUF_SIZE)
    {
        (void)FM225_RingBuf_Pop(&byte);
        return;
    }
    ...
    payload_len = (length < FM225_MAX_RESPONSE_SIZE) ? length : FM225_MAX_RESPONSE_SIZE;
    memset(recv_buf, 0, sizeof(recv_buf));
    ...
    if (!FM225_RingBuf_Pop(&recv_checksum))
    {
        return;
    }
    ...
    {
    case FM225_RESP_NOTE:
        FM225_HandleNote(recv_buf, payload_len);
        break;
    case FM225_RESP_REPLY:
        FM225_HandleReply(recv_buf, payload_len);
        break;
    default:
        break;
    }
...
void FM225_EnrollFace(const char *name, FM225_FaceDirection_t direction)
{
    ...
    /* 用户操作优先: 若当前命令为后台初始化查询则强制取消 */
    if (active_command != FM225_CMD_NONE &&
        active_command != FM225_CMD_VERIFY &&
        active_command != FM225_CMD_ENROLL)
    {
        active_command     = FM225_CMD_NONE;
        active_cmd_deadline = 0;
        deferred_cmd       = FM225_CMD_NONE;
        ...
        FM225_RingBuf_Reset();
        rx_overflow        = false;
        FM225_UART_Recover();  /* 恢复 UART 接收中断，避免 error state 导致后续无法收包 */
    }
...
void FM225_VerifyFace(void)
{
        ...
        active_command != FM225_CMD_ENROLL)
    {
        ...
        FM225_RingBuf_Reset();
        rx_overflow        = false;
        FM225_UART_Recover();  /* 恢复 UART 接收中断 */
    }

    static const uint8_t data[2] = {0, 0};  /* 协议标准格式 (ESPHome 官方实现确认) */
...
void FM225_CancelCurrentTask(void)
{
    FM225_Command_t previous_cmd = active_command;
    ...
    if (previous_cmd == FM225_CMD_VERIFY || previous_cmd == FM225_CMD_ENROLL)
    {
        FM225_SendCommand(FM225_CMD_RESET, NULL, 0);
    }
}
