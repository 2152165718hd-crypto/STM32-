...
#define ONENET_PRODUCT_ID "3nJMD1yIKI"
#define ONENET_DEVICE_NAME "don1ng"
#define ONENET_TOKEN "version=2018-10-31&res=products%2F3nJMD1yIKI%2Fdevices%2Fdon1ng&et=1861891200&method=md5&sign=g4bHKRMmlJzyls%2BP5qFgRw%3D%3D"
#define ONENET_PROPERTY_POST_TOPIC "$sys/3nJMD1yIKI/don1ng/thing/property/post"
#define ONENET_PROPERTY_SET_TOPIC "$sys/3nJMD1yIKI/don1ng/thing/property/set"
#define ONENET_PROPERTY_SET_REPLY_TOPIC "$sys/3nJMD1yIKI/don1ng/thing/property/set_reply"
...
#define ONENET_PAYLOAD_BUF_SIZE 192U
...
static int OneNet_FormatPayload(char *buffer, uint16_t size, const OneNetTelemetry_t *data)
{
    ...
    written = snprintf(buffer,
                       size,
                       "{\"id\":\"%lu\",\"params\":{\"AV_ALM\":{\"value\":%s},\"Flame\":{\"value\":%s},\"Hum\":{\"value\":%ld},\"Smoke\":{\"value\":%ld},\"Temp\":{\"value\":%ld}}}",
                       (unsigned long)message_id,
                       (data->av_alm != 0U) ? "true" : "false",
                       (data->flame != 0U) ? "true" : "false",
                       (long)data->hum,
                       (long)data->smoke,
                       (long)data->temp);
...
static void OneNet_HandlePropertySetPayload(const char *payload)
{
    ...
    if (params != NULL)
    {
        alarm_item = cJSON_GetObjectItem(params, "AV_ALM");
    }

    if ((alarm_item != NULL) &&
        (OneNet_ParseAlarmState(alarm_item, &alarm_on) != 0U) &&
