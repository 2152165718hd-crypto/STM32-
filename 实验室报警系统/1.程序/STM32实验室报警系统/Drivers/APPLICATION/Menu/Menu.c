...
#define TELEMETRY_REPORT_PERIOD_MS 2000U
...
#define ALARM_REASON_FLAME_HIGH (1U << 5)
...
static SensorSnapshot_t g_sensor = {0, 0, 0U, 0U, 0U, 0U, 0U, 0U};
static ThresholdConfig_t g_threshold = {18, 30, 30, 80, 60, 60};
...
static uint8_t Menu_RawToInversePercent(uint16_t adc_raw);
...
static uint8_t Menu_RawToInversePercent(uint16_t adc_raw)
{
    uint32_t pct = ((uint32_t)(4095U - adc_raw) * 100U + 2047U) / 4095U;
    if (pct > 100U)
    {
        pct = 100U;
    }
...
static void Menu_NormalizeThresholds(void)
{
    ...
    g_threshold.flame_high_pct = Menu_ClampI32(g_threshold.flame_high_pct, PCT_MIN_VALUE, PCT_MAX_VALUE);
...
static void Menu_SampleSensors(void)
{
    ...
    g_sensor.flame_raw = Flame_GetAO_Raw();
    g_sensor.smoke_raw = MQ2_Smoke_GetAO_Raw();
    g_sensor.flame_pct = Menu_RawToInversePercent(g_sensor.flame_raw);
...
static void Menu_UpdateAutoAlarm(void)
{
    ...
    Menu_UpdateHighTriggerU8(&g_alarm.flame_high_active, g_sensor.flame_pct, g_threshold.flame_high_pct, PCT_HYSTERESIS);
    ...
    if (g_alarm.flame_high_active != 0U)
    {
        reason_flags |= ALARM_REASON_FLAME_HIGH;
    }
...
static const char *Menu_AlarmReasonText(uint8_t reason_flags)
{
    ...
    if (reason_flags == ALARM_REASON_FLAME_HIGH)
    {
        return "FLAME_HIGH";
    }
...
static void Menu_FormatDataMessage(char *buffer, uint16_t size, uint8_t with_ok_prefix)
{
    if (with_ok_prefix != 0U)
    {
        snprintf(buffer, size,
                 "OK DATA T=%ld H=%ld S=%u F=%u AO=%u AM=%s AS=%s R=%s\r\n",
                 (long)g_sensor.temperature, (long)g_sensor.humidity,
                 (unsigned int)g_sensor.smoke_pct, (unsigned int)g_sensor.flame_pct,
    ...
    {
        snprintf(buffer, size,
                 "DATA T=%ld H=%ld S=%u F=%u AO=%u AM=%s AS=%s R=%s\r\n",
                 (long)g_sensor.temperature, (long)g_sensor.humidity,
                 (unsigned int)g_sensor.smoke_pct, (unsigned int)g_sensor.flame_pct,
                 (unsigned int)g_alarm.output_on,
                 Menu_AlarmModeText(g_alarm.mode),
...
static uint8_t Menu_SendPeriodicData(void)
{
    ...
    if (OneNet_PublishTelemetry(&telemetry) != 0U)
    {
        snprintf(history_line,
                 sizeof(history_line),
                 "T=%ld H=%ld S=%ld F=%u A=%u",
                 (long)telemetry.temp,
                 (long)telemetry.hum,
                 (long)telemetry.smoke,
                 (unsigned int)telemetry.flame,
...
static void Menu_ProcessCommandLine(uint8_t link_id, char *line)
{
    ...
    if (strcmp(cmd_buf, "GET THRESHOLD") == 0)
    {
        snprintf(tx_buf, sizeof(tx_buf),
                 "OK THR TL=%ld TH=%ld HL=%ld HH=%ld SH=%ld FH=%ld\r\n",
                 (long)g_threshold.temp_low, (long)g_threshold.temp_high,
                 (long)g_threshold.humi_low, (long)g_threshold.humi_high,
                 (long)g_threshold.smoke_high_pct, (long)g_threshold.flame_high_pct);
        ESP_SendString(link_id, tx_buf);
        return;
    }
    ...
    *eq = '\0';
    ...
    else if (strcmp(key, "FLAME_HIGH") == 0)
    {
        g_threshold.flame_high_pct = number;
    }
    else
    {
...
static void Menu_PageFlame(KeyEvent_t key, uint8_t *exit_flag)
{
    ...
    Menu_DrawPageHeader("Flame Sensor");

    snprintf(line, sizeof(line), "Value:%u%%", (unsigned int)g_sensor.flame_pct);
    OLED_ShowString(0, 18, line, OLED_6X8);

    snprintf(line, sizeof(line), "ADC Raw:%u", (unsigned int)g_sensor.flame_raw);
    OLED_ShowString(0, 27, line, OLED_6X8);

    snprintf(line, sizeof(line), "Thr:%ld%%", (long)g_threshold.flame_high_pct);
    OLED_ShowString(0, 36, line, OLED_6X8);

    Menu_DrawPercentBar(g_sensor.flame_pct, 50);
}
...
static void Menu_PageWiFiDebug(KeyEvent_t key, uint8_t *exit_flag)
{
    ...
    snprintf(line, sizeof(line), "Upload:%lus", (unsigned long)(TELEMETRY_REPORT_PERIOD_MS / 1000U));
...
static void Menu_BuildMenus(void)
{
    ...
    MF_AddCustomPage(g_menu_sensor, "Flame", Menu_PageFlame);
    ...
    MF_AddValue(g_menu_threshold, "Flame High", &g_threshold.flame_high_pct, PCT_MIN_VALUE, PCT_MAX_VALUE, 1, "%", Menu_OnThresholdChanged);
...
void Menu_SystemLoop(void)
{
    ...
    if ((g_report_pending != 0U) && ((int32_t)(now - g_report_due_tick) >= 0))
    {
        ...
        {
            g_report_due_tick = now + REMOTE_REPORT_DELAY_MS;
        }
    }
    else if ((now - g_last_report_tick) >= TELEMETRY_REPORT_PERIOD_MS)
    {
        g_last_report_tick = now;
