...
/* ========== 私有变量 ========== */
static UART_HandleTypeDef huart_bt; /* 蓝牙 UART 句柄        */
static uint8_t bt_rx_byte;          /* 单字节中断接收缓冲    */
static volatile uint8_t bt_rx_rearm_pending = 0u;
...
static void Bluetooth_TryStartReceiveIT(void)
{
    HAL_StatusTypeDef status;

    if (huart_bt.RxState == HAL_UART_STATE_BUSY_RX)
    {
        bt_rx_rearm_pending = 0u;
        return;
    }

    status = HAL_UART_Receive_IT(&huart_bt, &bt_rx_byte, 1);
    if (status == HAL_OK)
    {
        bt_rx_rearm_pending = 0u;
    }
    ...
    {
        bt_rx_rearm_pending = 1u;
    }
}

static void Bluetooth_ServiceRxRearm(void)
{
    if (bt_rx_rearm_pending == 0u)
    {
        return;
    }

    if (huart_bt.gState == HAL_UART_STATE_BUSY_TX)
    {
        return;
    }

    Bluetooth_TryStartReceiveIT();
}
...
/**
 * @brief  初始化 BT_UART (9600-8-N-1)、GPIO、NVIC 并开启接收中断
 */
void Bluetooth_Init(void)
{
    ...
    /* 6. 使能 UART 中断 */
    HAL_NVIC_SetPriority(BT_UART_IRQn, 3, 0);
...
/**
 * @brief  读取蓝牙连接状态（STATE 引脚）
 */
void Bluetooth_Enable(uint8_t enable)
{
    HAL_GPIO_WritePin(BT_GPIO_PORT,
                      BT_EN_PIN,
                      (enable != 0u) ? BT_EN_ACTIVE_STATE
                                     : ((BT_EN_ACTIVE_STATE == GPIO_PIN_SET) ? GPIO_PIN_RESET
                                                                             : GPIO_PIN_SET));
}

uint8_t Bluetooth_IsConnected(void)
{
    return (HAL_GPIO_ReadPin(BT_GPIO_PORT, BT_STATE_PIN) == GPIO_PIN_SET) ? 1U : 0U;
}
...
{
    Bluetooth_ServiceRxRearm();
    (void)HAL_UART_Transmit(&huart_bt, (uint8_t *)data, len, 200);
    Bluetooth_ServiceRxRearm();
}
...
void BLUETOOTH_ErrorCallback(void)
{
    __HAL_UART_CLEAR_PEFLAG(&huart_bt);
    huart_bt.ErrorCode = HAL_UART_ERROR_NONE;
    bt_rx_rearm_pending = 1u;
    Bluetooth_TryStartReceiveIT();
}
