...
#define BT_TX_PIN GPIO_PIN_10 /* PB10  -> 模块 RX */
#define BT_RX_PIN GPIO_PIN_11 /* PB11  <- 模块 TX */
#define BT_STATE_PIN GPIO_PIN_12 /* PB12  <- 模块 STATE */
#define BT_EN_PIN GPIO_PIN_13
#define BT_EN_ACTIVE_STATE GPIO_PIN_SET
#define BT_UART USART3
#define BT_UART_IRQn USART3_IRQn
#define BT_UART_IRQHandler USART3_IRQHandler
#define BT_UART_CLK_ENABLE() __HAL_RCC_USART3_CLK_ENABLE()
#define BT_GPIO_CLK_ENABLE() __HAL_RCC_GPIOB_CLK_ENABLE()
...
/**
 * @brief 初始化 JDY-31 蓝牙模块。
 * @note 内部会完成 BT_UART、GPIO、NVIC 与接收中断的配置。
 */
 ...
 *       if (huart->Instance == BT_UART) BLUETOOTH_RxCpltCallback();
 *   }
 * @endcode
 */
