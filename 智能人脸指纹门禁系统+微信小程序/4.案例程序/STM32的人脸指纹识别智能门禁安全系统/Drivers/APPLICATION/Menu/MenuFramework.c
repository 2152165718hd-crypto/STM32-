...
static void MF_HandleValueEdit(KeyEvent_t key)
{
    ...
    OLED_ShowString(4, MF_TITLE_HEIGHT + 4, (char *)editing_item->text, MF_FONT);

    snprintf(line_buf, sizeof(line_buf), "< %ld %s >", (long)*pval, u);
    OLED_ShowString(16, MF_TITLE_HEIGHT + 24, line_buf, MF_FONT);

    // 底部提示
    OLED_ShowString(4, MF_SCREEN_HEIGHT - 12, "OK:Confirm  B:Back", OLED_6X8);
}
...
{
    /* 处于菜单浏览模式 且 导航栈为空，即为主菜单 */
    return (nav_mode == NAV_MENU && nav_depth == -1) ? 1 : 0;
}
