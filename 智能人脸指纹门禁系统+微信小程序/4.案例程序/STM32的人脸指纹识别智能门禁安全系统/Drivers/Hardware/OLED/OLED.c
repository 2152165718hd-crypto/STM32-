 ...
 * 才会将显存数组的数据发送到OLED硬件，进行显�?
 */
uint8_t OLED_DisplayBuf[8][128];
#if OLED_USE_HARDWARE_I2C
static uint8_t oled_i2c_tx_buf[129];
#endif
static char oled_printf_buf[256];
/*********************全局变量*/
 ...
 * �?   数：要写入SCL的电平值，范围�?/1
 * �?�?值：�?
 * �?   明：当上层函数需要写SCL时，此函数会被调�?
 *           用户需要根据参数传入的值，将SCL置为高电平或者低电平
 *           当参数传�?时，置SCL为低电平，当参数传入1时，置SCL为高电平
 */
 ...
 * �?   数：要写入SDA的电平值，范围�?/1
 * �?�?值：�?
 * �?   明：当上层函数需要写SDA时，此函数会被调�?
 *           用户需要根据参数传入的值，将SDA置为高电平或者低电平
 *           当参数传�?时，置SDA为低电平，当参数传入1时，置SDA为高电平
 */
 ...
 * �?   数：�?
 * �?�?值：�?
 * �?   明：当上层函数需要初始化时，此函数会被调�?
 *           用户需要将SCL和SDA引脚初始化为开漏模式，并释放引�?
 */
...
static void OLED_I2C1_Init(void)
{
	...
	*/
	...
	/* 配置I2C参数 */
	hi2c1.Instance = I2C1;
	hi2c1.Init.ClockSpeed = 400000; // 400kHz快速模�?
 ...
 * �?   数：�?
 * �?�?值：�?
 * �?   明：初始化I2C1硬件，使用PB6(SCL)和PB7(SDA)
 */
void OLED_GPIO_Init(void)
{
	uint32_t i, j;
...
void OLED_I2C_Start(void)
{
	...
	OLED_W_SCL(0); // 起始后把SCL也拉低，即为了占用总线，也为了方便总线时序的拼�?
}
...
void OLED_I2C_Stop(void)
{
	...
	OLED_W_SDA(1); // 在SCL高电平期间，释放SDA，产生终止信�?
}

/**
 * �?   数：I2C发送一个字节（软件I2C�?
 * �?   数：Byte 要发送的一个字节数据，范围�?x00~0xFF
 * �?�?值：�?
 */
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	for (i = 0; i < Count; i++)
	{
		OLED_I2C_SendByte(Data[i]); // 依次发送Data的每一个数�?
	}
	OLED_I2C_Stop(); // I2C终止
}
...
{
	uint8_t buf[2];
	buf[0] = 0x00;	  // 控制字节，表示即将写命令
	buf[1] = Command; // 命令�?

	HAL_I2C_Master_Transmit(&OLED_I2C_INSTANCE, OLED_I2C_ADDR, buf, 2, OLED_I2C_TIMEOUT);
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	oled_i2c_tx_buf[0] = 0x40; // 控制字节，表示即将写数据
	for (i = 0; i < Count; i++)
	{
		oled_i2c_tx_buf[i + 1] = Data[i];
	}

	HAL_I2C_Master_Transmit(&OLED_I2C_INSTANCE, OLED_I2C_ADDR, oled_i2c_tx_buf, Count + 1, OLED_I2C_TIMEOUT);
}
...
void OLED_Init(void)
{
	OLED_GPIO_Init(); // 先调用底层的端口初始�?

	/*写入一系列的命令，对OLED进行初始化配�?*/
	OLED_WriteCommand(0xAE); // 设置显示开�?关闭�?xAE关闭�?xAF开�?
	...
	OLED_WriteCommand(0x40); // 设置显示开始行�?x40~0x7F

	OLED_WriteCommand(0xA1); // 设置左右方向�?xA1正常�?xA0左右反置

	OLED_WriteCommand(0xC8); // 设置上下方向�?xC8正常�?xC0上下反置
	...
	OLED_WriteCommand(0xA6); // 设置正常/反色显示�?xA6正常�?xA7反色
...
/**
 * �?   数：OLED设置显示光标位置
 * �?   数：Page 指定光标所在的页，范围�?~7
 * �?   数：X 指定光标所在的X轴坐标，范围�?~127
 * �?�?值：�?
 * �?   明：OLED默认的Y轴，只能8个Bit为一组写入，�?页等�?个Y轴坐�?
 */
 ...
 * �?   数：判断指定点是否在指定角度内部
 * �?   数：X Y 指定点的坐标
 * �?   数：StartAngle EndAngle 起始角度和终止角度，范围�?180~180
 *           水平向右�?度，水平向左�?80度或-180度，下方为正数，上方为负数，顺时针旋�?
 * �?�?值：指定点是否在指定角度内部�?：在内部�?：不在内�?
 */
 ...
 * �?   数：X 指定区域左上角的横坐标，范围�?32768~32767，屏幕区域：0~127
 * �?   数：Y 指定区域左上角的纵坐标，范围�?32768~32767，屏幕区域：0~63
 * �?   数：Width 指定区域的宽度，范围�?~128
 * �?   数：Height 指定区域的高度，范围�?~64
 * �?�?值：�?
 * �?   明：此函数会至少更新参数指定的区�?
...
void OLED_Clear(void)
{
	...
	for (j = 0; j < 8; j++) // 遍历8�?
	{
		...
		{
			OLED_DisplayBuf[j][i] = 0x00; // 将显存数组数据全部清�?
		}
	}
}
...
/**
 * �?   数：OLED显示一个字�?
 * �?   数：X 指定字符左上角的横坐标，范围�?32768~32767，屏幕区域：0~127
 * �?   数：Y 指定字符左上角的纵坐标，范围�?32768~32767，屏幕区域：0~63
 * �?   数：Char 指定要显示的字符，范围：ASCII码可见字�?
 * �?   数：FontSize 指定字体大小
...
/**
 * �?   数：OLED显示字符串（支持ASCII码和中文混合写入�?
 * �?   数：X 指定字符串左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
 * �?   数：Y 指定字符串左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
 * �?   数：String 指定要显示的字符串，范围：ASCII码可见字符或中文字符组成的字符串
 * �?   数：FontSize 指定字体大小
 *           范围：OLED_8X16		�?像素，高16像素
 *                 OLED_6X8		�?像素，高8像素
 * �?�?值：�?
 ...
 *           当字体大小为OLED_8X16时，中文字符�?6*16点阵正常显示
 *           当字体大小为OLED_6X8时，中文字符�?*8点阵显示'?'
 * �?   明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
void OLED_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
	...
	while (String[i] != '\0') // 遍历字符�?
	{
		...
		{
			CharLength = 1;				 // 字符�?字节
			SingleChar[0] = String[i++]; // 将第一个字节写入SingleChar�?个位置，随后i指向下一个字�?
		...
		else if ((String[i] & 0xE0) == 0xC0) // 第一个字节为110xxxxx
		{
			...
			SingleChar[1] = String[i++]; // 将第二个字节写入SingleChar�?个位置，随后i指向下一个字�?
		...
		else // 否则，即多字节字�?
		{
			...
			else if (FontSize == OLED_6X8) // 给定字体�?*8点阵
			{
				/*空间不足，此位置显示'?'*/
...
/**
 * �?   数：OLED显示十六进制数字（十六进制，正整数）
 * �?   数：X 指定数字左上角的横坐标，范围�?32768~32767，屏幕区域：0~127
 * �?   数：Y 指定数字左上角的纵坐标，范围�?32768~32767，屏幕区域：0~63
 * �?   数：Number 指定要显示的数字，范围：0x00000000~0xFFFFFFFF
 * �?   数：Length 指定数字的长度，范围�?~8
 * �?   数：FontSize 指定字体大小
 *           范围：OLED_8X16		�?像素，高16像素
 *                 OLED_6X8		�?像素，高8像素
 * �?�?值：�?
...
/**
 * �?   数：OLED显示图像
 * �?   数：X 指定图像左上角的横坐标，范围�?32768~32767，屏幕区域：0~127
 * �?   数：Y 指定图像左上角的纵坐标，范围�?32768~32767，屏幕区域：0~63
 * �?   数：Width 指定图像的宽度，范围�?~128
 * �?   数：Height 指定图像的高度，范围�?~64
 * �?   数：Image 指定要显示的图像
 * �?�?值：�?
 * �?   明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
...
/**
 * �?   数：OLED使用printf函数打印格式化字符串（支持ASCII码和中文混合写入�?
 * �?   数：X 指定格式化字符串左上角的横坐标，范围�?32768~32767，屏幕区域：0~127
 * �?   数：Y 指定格式化字符串左上角的纵坐标，范围�?32768~32767，屏幕区域：0~63
 * �?   数：FontSize 指定字体大小
 *           范围：OLED_8X16		�?像素，高16像素
 *                 OLED_6X8		�?像素，高8像素
 * �?   数：format 指定要显示的格式化字符串，范围：ASCII码可见字符或中文字符组成的字符串
 * �?   数：... 格式化字符串参数列表
 * �?�?值：�?
 * �?   明：显示的中文字符需要在OLED_Data.c里的OLED_CF16x16数组定义
 *           未找到指定中文字符时，会显示默认图形（一个方框，内部一个问号）
...
void OLED_Printf(int16_t X, int16_t Y, uint8_t FontSize, char *format, ...)
{
    ...
    va_end(arg);
    OLED_ShowString(X, Y, oled_printf_buf, FontSize);
}

/**
 * �?   数：OLED在指定位置画一个点
 * �?   数：X 指定点的横坐标，范围�?32768~32767，屏幕区域：0~127
...
/**
 * �?   数：OLED画线
 * �?   数：X0 指定一个端点的横坐标，范围�?32768~32767，屏幕区域：0~127
 * �?   数：Y0 指定一个端点的纵坐标，范围�?32768~32767，屏幕区域：0~63
 * �?   数：X1 指定另一个端点的横坐标，范围�?32768~32767，屏幕区域：0~127
 * �?   数：Y1 指定另一个端点的纵坐标，范围�?32768~32767，屏幕区域：0~63
 * �?�?值：�?
 * �?   明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
 */
...
/**
 * �?   数：OLED矩形
 * �?   数：X 指定矩形左上角的横坐标，范围�?32768~32767，屏幕区域：0~127
 * �?   数：Y 指定矩形左上角的纵坐标，范围�?32768~32767，屏幕区域：0~63
 * �?   数：Width 指定矩形的宽度，范围�?~128
 * �?   数：Height 指定矩形的高度，范围�?~64
 * �?   数：IsFilled 指定矩形是否填充
 *           范围：OLED_UNFILLED		不填�?
 *                 OLED_FILLED			填充
 * �?�?值：�?
...
/**
 * �?   数：OLED三角�?
 * �?   数：X0 指定第一个端点的横坐标，范围�?32768~32767，屏幕区域：0~127
 * �?   数：Y0 指定第一个端点的纵坐标，范围�?32768~32767，屏幕区域：0~63
 * �?   数：X1 指定第二个端点的横坐标，范围�?32768~32767，屏幕区域：0~127
 * �?   数：Y1 指定第二个端点的纵坐标，范围�?32768~32767，屏幕区域：0~63
 * �?   数：X2 指定第三个端点的横坐标，范围�?32768~32767，屏幕区域：0~127
 * �?   数：Y2 指定第三个端点的纵坐标，范围�?32768~32767，屏幕区域：0~63
 * �?   数：IsFilled 指定三角形是否填�?
 *           范围：OLED_UNFILLED		不填�?
 *                 OLED_FILLED			填充
 * �?�?值：�?
...
/**
 * �?   数：OLED画圆
 * �?   数：X 指定圆的圆心横坐标，范围�?32768~32767，屏幕区域：0~127
...
/**
 * �?   数：OLED画椭�?
 * �?   数：X 指定椭圆的圆心横坐标，范围：-32768~32767，屏幕区域：0~127
 * �?   数：Y 指定椭圆的圆心纵坐标，范围：-32768~32767，屏幕区域：0~63
 * �?   数：A 指定椭圆的横向半轴长度，范围�?~255
 * �?   数：B 指定椭圆的纵向半轴长度，范围�?~255
 * �?   数：IsFilled 指定椭圆是否填充
 *           范围：OLED_UNFILLED		不填�?
 *                 OLED_FILLED			填充
 * �?�?值：�?
 ...
 * �?   数：Radius 指定圆弧的半径，范围�?~255
 * �?   数：StartAngle 指定圆弧的起始角度，范围�?180~180
 *           水平向右�?度，水平向左�?80度或-180度，下方为正数，上方为负数，顺时针旋�?
 * �?   数：EndAngle 指定圆弧的终止角度，范围�?180~180
 *           水平向右�?度，水平向左�?80度或-180度，下方为正数，上方为负数，顺时针旋�?
 * �?   数：IsFilled 指定圆弧是否填充，填充后为扇�?
 *           范围：OLED_UNFILLED		不填�?
 *                 OLED_FILLED			填充
 * �?�?值：�?
 * �?   明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
