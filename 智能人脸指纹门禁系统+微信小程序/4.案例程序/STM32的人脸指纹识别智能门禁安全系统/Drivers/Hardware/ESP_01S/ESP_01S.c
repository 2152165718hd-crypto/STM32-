...
static UART_HandleTypeDef huart1_esp;
...
ESP_Status_t ESP_SendAT(const char *cmd, const char *ack, uint32_t timeout)
{
    ...
    HAL_UART_Transmit(&huart1_esp, (uint8_t *)cmd, (uint16_t)strlen(cmd), 1000U);
...
ESP_Status_t ESP_SendData(uint8_t link_id, const uint8_t *data, uint16_t len)
{
    ...
    HAL_UART_Transmit(&huart1_esp, (uint8_t *)data, len, 3000U);
