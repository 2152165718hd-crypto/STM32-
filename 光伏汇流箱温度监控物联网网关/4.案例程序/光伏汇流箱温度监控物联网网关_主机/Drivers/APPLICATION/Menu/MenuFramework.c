...
static uint8_t MF_IsCursorAnimatingInternal(void)
{
    float target_y;
    float delta;
    int8_t rel;

    if ((nav_mode != NAV_MENU) || (cur_menu == NULL) || (cur_menu->item_count == 0U))
    {
        return 0U;
    }

    rel = cur_menu->select_index - cur_menu->scroll_offset;
    target_y = (float)(MF_TITLE_HEIGHT + rel * MF_LINE_HEIGHT);
    delta = target_y - anim_cursor_y;
    if (delta < 0.0f)
    {
        delta = -delta;
    }

    return (delta >= MF_ANIM_EPSILON) ? 1U : 0U;
}
...
static void MF_RenderMenuList(void)
{
    ...
    if ((target_y - anim_cursor_y <= MF_ANIM_EPSILON) &&
        (target_y - anim_cursor_y >= -MF_ANIM_EPSILON))
    {
...
{
    return MF_IsCursorAnimatingInternal();
}

uint8_t MF_IsCustomPageActive(void)
{
    return (nav_mode == NAV_CUSTOM_PAGE) ? 1U : 0U;
}
