...
/**
 * 函    数：OLED硬件I2C初始化（I2C1）
 * 参    数：无
 * 返 回 值：无
 * 说    明：初始化I2C1硬件，使用PB6(SCL)和PB7(SDA)
 */
void OLED_GPIO_Init(void)
{
	uint32_t i, j;
...
{
	uint8_t i;
	uint8_t buf[129]; // 控制字节 + 128字节数据

	buf[0] = 0x40; // 控制字节，表示即将写数据
	for (i = 0; i < Count; i++)
	{
		buf[i + 1] = Data[i];
	}

	HAL_I2C_Master_Transmit(&OLED_I2C_INSTANCE, OLED_I2C_ADDR, buf, Count + 1, OLED_I2C_TIMEOUT);
}
